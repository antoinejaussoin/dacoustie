	...
	function setVariables() {
		...
		if ($this->_app->_session->get( 'isOK:' . $this->_sTask ) === false) {
			...
			if (is_array($postData)) {
				...
				$this->current_url = str_replace('&#38;','&',$postData['current_url']);
		...
		$this->contactinformations = $model->readContactInformations( $pf );
		...
		foreach($fields as $field_key=>$field) {
			...
			switch($field->field_type) {
				...
				case 'RL' :
					...
					$count_values = count($field_values);
					for($i=0;$i<$count_values;$i++) {
						$field->html_tag .= '<div id="div_' . $field->name . $i . '" class="' . $field->name . '" ' . $field->field_parameters . '><input type="radio" id="' . $field->name . $i . '" class="' . $field->name . '" name="' . $field->name . '" value="' . $field_values[$i] . '" '.($postData_field_value == $field_values[$i]?'checked="checked"':'').' /><label for="' . $field->name . $i . '">&nbsp;' . $field_values[$i] . '</label></div>';
					}
					break;
				case 'DT' :
					// Date
				...
				case 'SP' :
					// Separator
					$field->html_label = '<span class="aiContactSafe_label" id="aiContactSafe_label_' . $field->name . '" ' . $field->label_parameters . '>' . $field->field_label . '</span>';
					$field->html_tag = '<div id="sp_' . $field->name . '" ' . $field->field_parameters . '>' . (is_null($postData_field_value)?'':$postData_field_value) . '<input type="hidden" name="' . $field->name . '" id="' . $field->name . '" ' . $field->field_parameters . ' value="' . $postData_field_value . '" /></div>';
					break;
				case 'FL' :
					// File
					$field->html_label = '<span class="aiContactSafe_label" id="aiContactSafe_label_' . $field->name . '" ' . $field->label_parameters . '><label for="' . $field->name . '" ' . $field->label_parameters . '>' . $field->field_label . '</label></span>';
					...
					if ( is_array($postData) ) {
						if ( array_key_exists($field->name.'_attachment_id', $postData) && strlen($postData[$field->name.'_attachment_id']) > 0 ) {
							...
							$field->html_tag .= '<input type="text" name="' . $field->name . '_attachment_name" id="' . $field->name . '_attachment_name" ' . $field->field_parameters . ' value="' . $postData[$field->name.'_attachment_name'] . '" readonly="readonly" />';
					...
					$field->html_tag .= '<input type="hidden" name="' . $field->name . '_attachment_id" id="' . $field->name . '_attachment_id" value="' . $postData[$field->name.'_attachment_id'] . '" />';
					...
					$field->html_tag .= '<iframe id="iframe_upload_file_' . $this->profile->id.'_file_'.$field->name.'" name="iframe_upload_file_' . $this->profile->id.'_file_'.$field->name.'" src="#" style="width:0;height:0;border:0px solid #FFF;display:none;"></iframe>';
					break;
				case 'NO' :
					// Number
					$field->html_label = '<span class="aiContactSafe_label" id="aiContactSafe_label_' . $field->name . '" ' . $field->label_parameters . '><label for="' . $field->name . '" ' . $field->label_parameters . '>' . $field->field_label . '</label></span>';
			...
			if (strlen(trim($field->html_tag)) > 0) {
				if (strlen(trim($field->field_sufix)) > 0) {
					$field->html_tag = '<span class="aiContactSafe_sufix" id="' . $field->name . '_sufix">'.trim($field->field_sufix).'</span>'.$field->html_tag;
				}
				if (strlen(trim($field->field_prefix)) > 0) {
					$field->html_tag = $field->html_tag.'<span class="aiContactSafe_prefix" id="' . $field->name . '_prefix">'.trim($field->field_prefix).'</span>';
				}
			}
		...
		if ($use_css) {
			...
			if (strlen($cssFile) > 0) {
				...
				if (JFile::exists($tPath)) {
					...
					$nameCssFile = JURI::root().'components/com_aicontactsafe/views/message/tmpl/'.$cssFile;
				}
			...
			switch($this->profile->captcha_type) {
				case 0:
					...
					<div id="div_captcha">
						...
						<div id="div_captcha_img_<?php echo $this->profile->id; ?>" style="width:<?php echo $this->profile->captcha_width; ?>px;height:<?php echo $this->profile->captcha_height; ?>px;">...</div>
