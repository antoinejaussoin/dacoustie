...
class AiContactSafeModelProfiles extends AiContactSafeModelDefault {
	...
	function checkBeforeWrite($postData) {
		...
		foreach($postData as $key=>$value){
			if (substr($key,0,strlen('order_field_')) == 'order_field_') {
				$fields_id[] = str_replace('order_field_','',$key);
			}
		}
	...
	function writeOtherTables( $postData = array(), $id = 0 ) {
		...
		$mail_file = JPATH_ROOT.DS.'components'.DS.'com_aicontactsafe'.DS.'views'.DS.'mail'.DS.'tmpl'.DS.'mail_'.$id.'.php';
	...
	function getContactInformation( $profile_id = 0 ) {
		...
		$contact_info['required_field_notification'] = JText::_( 'Fields marked with' ).' %mark% '.JText::_( 'are required' ).'.';
	...
	function getFields( $active_fields = '', $profile_id = 0 ) {
		...
		$query = 'SELECT fields_order FROM #__aicontactsafe_profiles WHERE id = '.$profile_id;
		$db->setQuery( $query );
	...
	function deleteData() {
		...
		if (is_array($css_ids)) {
			...
			foreach($css_ids as $css_id){
				$css_file = JPATH_ROOT.DS.'components'.DS.'com_aicontactsafe'.DS.'views'.DS.'message'.DS.'tmpl'.DS.'profile_css_'.$css_id.'.css';
	...
	function copyProfile() {
		...
		foreach($other_info as $info) {
			switch($info->info_key) {
				case 'contact_info' :
					JRequest::setVar('contact_info', $info->info_value, 'post', true);
					break;
				case 'meta_description' :
	...
	function readMailTemplate( $id = 0 ) {
		...
		if ( $id > 0 ) {
			...
			if (JFile::exists($mail_file)) {
				...
				$mail_file = JPATH_ROOT.DS.'components'.DS.'com_aicontactsafe'.DS.'views'.DS.'mail'.DS.'tmpl'.DS.'mail.php';
				if (JFile::exists($mail_file)) {
					$mail_code = JFile::read($mail_file);
				}
			}
