...
class AiContactSafeModelMessage extends AiContactSafeModelDefault {
	...
	function uploadFile() {
		...
		if ( strlen(trim($file['name'])) > 0 ) {
			...
			if ( $errorType == 0 ) {
				...
				$newFile->name = '';
				$newFile->r_id = $r_id;
				$newFile->date_added = JHTML::_('date', 'now', '%Y-%m-%d %H:%M:%S');
				$newFile->last_update = JHTML::_('date', 'now', '%Y-%m-%d %H:%M:%S');
				$newFile->published = 1;
		...
		if(!$this->fields) {
			...
			foreach($fields_order as $field_id) {
				if (array_key_exists($field_id, $unsorted_fields)) {
					$this->fields[] = $unsorted_fields[$field_id];
	...
	function securityCheck($postData) {
		...
		// test for captcha
		if ( $isOK ) {
			if (array_key_exists('captcha-code',$postData) && ( $profile->use_captcha == 1 || ($profile->use_captcha == 2 && $this->_user_id == 0) )) {
	...
	function checkBeforeWrite($postData) {
		...
		$postData['date_added'] = JHTML::_('date', 'now', '%Y-%m-%d %H:%M:%S');
		$postData['last_update'] = JHTML::_('date', 'now', '%Y-%m-%d %H:%M:%S');
	...
	function sendEmail() {
			...
			switch($field->field_type) {
				...
				case 'SB' :
					...
					if ($postData[$field->name] > 0) {
						...
						if ($field->send_message) {
							$email_recipients[] = $emails[0]->email;
						}
					} else {
						$field_value = '...';
					}
		...
		if ( $spam_control == 1 or $spam_control == 2 ) {
			...
			if ($profile->use_mail_template) {
				...
				if (JFile::exists($tPath)) {
					...
					$tPath = JPATH_ROOT.DS.'components'.DS.'com_aicontactsafe'.DS.'views'.DS.'mail'.DS.'tmpl'.DS.'mail_'.$profile->id.'.php';
					if (JFile::exists($tPath)) {
						$nameMailTemplate = $tPath;
					} else {
						$nameMailTemplate = JPATH_ROOT.DS.'components'.DS.'com_aicontactsafe'.DS.'views'.DS.'mail'.DS.'tmpl'.DS.'mail.php';
					}
				}
		...
		if ($profile->record_message && ($spam_control == 1 or $spam_control == 2 or $spam_control == 3)) {
			...
			$query = 'UPDATE #__aicontactsafe_messagefiles SET message_id = ' . $dataRow->id . ' WHERE message_id = 0 and r_id = '.$postData['r_id'];
			$db->setQuery($query);
			$db->query();
			...
			if ($profile->record_fields) {
				...
				foreach($fields as $field_key=>$field) {
					...
					$fieldRow->date_added = JHTML::_('date', 'now', '%Y-%m-%d %H:%M:%S');
					$fieldRow->last_update = JHTML::_('date', 'now', '%Y-%m-%d %H:%M:%S');
