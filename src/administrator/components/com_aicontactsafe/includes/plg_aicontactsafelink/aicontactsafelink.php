		...
		if ($no_links>1) {
			for($i=1;$i<$no_links;$i++) {
				...
				if ( $this->checkProfile($parameters['pf']) ) {
					...
					$plugin_link .= '<form name="aiContactSafeCall_'.$parameters['pf'].'" id="aiContactSafeCall_'.$parameters['pf'].'" action="'.$parameters['action'].'" method="post">';
					// this is the link
					$plugin_link .= '<a class="aiContactSafeLink" href="#" onclick="document.aiContactSafeCall_'.$parameters['pf'].'.submit();">'.$parameters['text'].'</a>';
				...
				$article->text = str_replace($initial_text, $plugin_link, $article->text);
			}
		}
	}
