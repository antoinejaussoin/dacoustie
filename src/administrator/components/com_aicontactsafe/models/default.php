...
class AiContactSafeModelDefault extends JModel {
	...
	function __construct( $default = array() )	{
		...
		$this->filter_string = $this->getSessionStateFromRequest( $this->_sTask.'filter_string', 'filter_string', '', 'string' );
	...
	function setRowValues(&$rowlist) {
		$n = count($rowlist);
		for ($i = 0; $i < $n; $i++ ) {
			$rowlist[$i]->edit = JRoute::_('index.php?option=com_aicontactsafe&sTask=' . $this->_sTask . '&task=edit&id=' . $rowlist[$i]->id);
		}
	}
	...
	function getWhere() {
		...
		if ( strlen($this->filter_string) > 0 ) {
			$where .= ' AND LOWER( ' . $this->filter_field . ' ) LIKE "%' . $this->filter_string . '%"';
		}
		return $where;
	}
