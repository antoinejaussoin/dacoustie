...
// 2.0.5 modifications in database 
	...
	$db->setQuery( $query );
	$db->query();
	$query = 'ALTER TABLE `#__aicontactsafe_messages` ADD `email_replay` varchar(100) NOT NULL default \'\' COMMENT \'the email address where the replay was sent\' AFTER `profile_id` ;';
	$db->setQuery( $query );
	$db->query();
	$query = 'ALTER TABLE `#__aicontactsafe_messages` ADD `subject_replay` text NOT NULL default \'\' COMMENT \'the subject of the replay\' AFTER `email_replay` ;';
	$db->setQuery( $query );
	$db->query();
	$query = 'ALTER TABLE `#__aicontactsafe_messages` ADD `message_replay` text NOT NULL default \'\' COMMENT \'the message of the replay\' AFTER `subject_replay` ;';
	$db->setQuery( $query );
	$db->query();
...
// 2.0.6 modifications in database 
	...
	$value = 'components'.DS.'com_aicontactsafe'.DS.'attachments';
	...
	if ($id == 0) {
		...
		$query = 'INSERT INTO `#__aicontactsafe_profiles` (`id`, `name`, `use_message_css`, `contact_form_width`, `contact_info_width`, `use_captcha`, `email_address`, `always_send_to_email_address`, `subject_prefix`, `record_message`, `custom_date_format`, `custom_date_years_back`, `custom_date_years_forward`, `required_field_mark`, `display_format`, `set_default`, `active_fields`, `captcha_width`, `captcha_height`, `captcha_bgcolor`, `captcha_backgroundTransparent`, `captcha_colors`, `name_field_id`, `email_field_id`, `subject_field_id`, `send_to_sender_field_id`, `date_added`, `last_update`, `published`, `checked_out`, `checked_out_time`) VALUES (2, \'Module form\', 1, 0, 0, 1, \'\', 1, \'\', 1, \'%d %B %Y\', 60, 0, \'( ! )\', 1, 1, \'0\', 180, 55, \'#FFFFFF\', 1, \'#FF0000;#00FF00;#0000FF\', 1, 2, 4, 6, \'2009-01-01 00:00:00\', \'2009-01-01 00:00:00\', 1, 0, \'0000-00-00\');';
		$db->setQuery( $query );
		$db->query();
	}
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			...
			<td width="50%" valign="top">
				...
				<br/>
				<b>English translation of instructions</b> : <br/>- Nic Irvine <a href="http://www.swanshops.com" target="_blank">www.swanshops.com</a><br/>
				<br/>
